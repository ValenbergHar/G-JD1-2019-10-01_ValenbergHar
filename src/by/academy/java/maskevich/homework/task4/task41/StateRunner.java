...
//		getCapital(country);
	...
	private static void getCapital(Country country) {
		((District) ((Region) country.getRegoins()).getDistricts()).getCities().forEach(city -> {
			if (city.getCityType().equals(CityType.CAPITAL_AND_DISTRICT_CENTR)) {
				System.out.println("Capital is " + city.getName());
			}
		});
	}
	...
	private static void getDistrictsCentres(Country country) {
		((District) ((Region) country.getRegoins()).getDistricts()).getCities().forEach(city -> {
			if (city.getCityType().equals(CityType.DISTRICT_CENTR)) {
				System.out.println("region centres " + city.getName());
			}
		});
